...
public:
    ...
    void getEvent(const QString &calendarId, const QString &eventId,
    ...
    void fetchColorDefinitions(std::function<void(QJsonObject calendarColors, QJsonObject eventColors, QString error)> callback);
    ...
    void patchEvent(const QString &calendarId, const QString &eventId, const QString &etag,
                     const QJsonObject &patchBody,
                     std::function<void(QJsonObject event, QString errorCode, QString errorMessage)> callback);

    // Creates a new event. No etag involved — there's nothing to conflict with yet.
    void insertEvent(const QString &calendarId, const QJsonObject &eventBody,
                      std::function<void(QJsonObject event, QString errorCode, QString errorMessage)> callback);

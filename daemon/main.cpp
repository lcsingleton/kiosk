...
QString resolveColor(const QString &raw, const QJsonObject &eventPalette, const QJsonObject &calendarPalette,
    ...
    warning = QStringLiteral("unknown color \"%1\" — use a literal hex instead (see GoogleColorNames.h for known names)").arg(raw);
    return QString();
}
...
void runSyncCycle(CalendarClient *client, const QVector<CalendarConfig> &calendars,
    ...
    for (const CalendarConfig &cal : calendars) {
        client->listEvents(cal.calendarId, windowStart, windowEnd,
...
int main(int argc, char *argv[]) {
    ...
    client->fetchColorDefinitions([=](QJsonObject calendarPalette, QJsonObject eventPalette, QString colorsError) {
        ...
        auto cycle = [client, calendars, resolvedPeople, eventColorIdToHex, calendarFallbackHex, snapshotPath, once]() {
            runSyncCycle(client, calendars, resolvedPeople, eventColorIdToHex, calendarFallbackHex, snapshotPath,
                         [once](bool hadError) {
                             if (once)

...
QPair<QString, QString> classifyFailure(QNetworkReply *reply, const QByteArray &body) {
    ...
    QString code;
    switch (status) {
    case 400: code = QStringLiteral("invalid_request"); break;
    case 401: case 403: code = QStringLiteral("auth_failure"); break;
    case 404: case 410: code = QStringLiteral("not_found"); break;
    case 412: code = QStringLiteral("conflict"); break;
    default: code = QStringLiteral("upstream_unavailable"); break;
    }
...
void CalendarClient::listEvents(const QString &calendarId, const QDateTime &timeMin, const QDateTime &timeMax,
    ...
    m_auth->accessToken([this, calendarId, timeMin, timeMax, callback](QString token, QString error) {
        ...
        *fetchPage = [this, calendarId, timeMin, timeMax, token, callback, fetchPage]
                     (QString pageToken, QJsonArray accumulated) {
            ...
            connect(reply, &QNetworkReply::finished, this, [reply, accumulated, callback, fetchPage]() mutable {
                reply->deleteLater();
                if (reply->error() != QNetworkReply::NoError) {
                    const QByteArray body = reply->readAll();
                    callback({}, QStringLiteral("Calendar API request failed: %1%2")
                        .arg(reply->errorString(), body.isEmpty() ? QString() : QStringLiteral(" — %1").arg(QString::fromUtf8(body))));
                    return;
                }
...
void CalendarClient::patchEvent(const QString &calendarId, const QString &eventId, const QString &etag,
                                 ...
                                 std::function<void(QJsonObject, QString, QString)> callback) {
    m_auth->accessToken([this, calendarId, eventId, etag, patchBody, callback](QString token, QString authError) {
        if (token.isEmpty()) {
            callback({}, QStringLiteral("auth_failure"), authError);
            return;
        }
        ...
        QNetworkReply *reply = m_nam.sendCustomRequest(request, "PATCH",
            QJsonDocument(patchBody).toJson(QJsonDocument::Compact));
        connect(reply, &QNetworkReply::finished, this, [reply, callback]() {
            reply->deleteLater();
            const QByteArray body = reply->readAll();
            if (reply->error() != QNetworkReply::NoError) {

...
class CalendarClient : public QObject
{
  ...
  public:
	...
	/// @param auth Provides the bearer token for every call below.
	/// @param delegatedAuth Optional fallback for the service-account
	/// invite limitation described above; nullptr disables it.
	/// @param parent Standard QObject ownership parent.
	explicit CalendarClient( GoogleAuth *auth, DelegatedAuth *delegatedAuth = nullptr, QObject *parent = nullptr );
	...
	void listEvents( const QString &calendarId,
	...
	void getEvent( const QString &calendarId,
	...
	void
	fetchColorDefinitions( std::function<void( QJsonObject calendarColors, QJsonObject eventColors, QString error )> callback );
	...
	void patchEvent( const QString &calendarId,
					 const QString &eventId,
					 const QString &etag,
  ...
  private:
	...
	void sendPatch( const QString &calendarId,
					const QString &eventId,
					const QString &etag,
					const QJsonObject &patchBody,
					const QString &token,
